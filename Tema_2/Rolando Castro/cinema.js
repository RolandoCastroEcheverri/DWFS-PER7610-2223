...
function show() {
	let salida = ""
	for (var i = 0; i < aSillas.length; i++) {
		for (var a = 0; a < aSillas.length; a++) {
			salida = salida + aSillas[i][a] + "  ";
		}
		salida = salida + "\n";
	}
	console.log(salida);
...
function order(requestSize) {
	...
	if (requestSize > aSillas.length) {
		...
		for (var i = 0; i < aSillas.length; i++) {
			var indices = [];
			for (var v = 0; v < aSillas.length; v++) {

				filaV = filaV + aSillas[i][v];
			}
			...
			for (var a = 0; a < aSillas.length; a++) {
				...
				if ((indices.length >= requestSize) & (aSillas[i][a] == "-")) {
